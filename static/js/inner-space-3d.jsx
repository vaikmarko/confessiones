...
const InnerSpace = ({ stories = [], onStoryClick }) => {
  ...
  if (hasError) {
    return (
      <div className="relative w-full h-64 rounded-xl overflow-hidden bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900">
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center text-white p-6">
            <div className="text-4xl mb-4">🌌</div>
            <h3 className="text-lg font-semibold mb-2">Your Inner Space</h3>
            <p className="text-sm text-purple-200 mb-4">
              {stories.length} stories in your space
            </p>
      ...
      {isLoading && (
        <div className="absolute inset-0 flex items-center justify-center text-white z-10">
          <div className="text-center">
            <div className="animate-spin w-8 h-8 border-2 border-purple-400 border-t-transparent rounded-full mb-2 mx-auto"></div>
            <p className="text-sm">Loading your inner space...</p>
          </div>
        </div>
      )}
      <div ref={mountRef} className="w-full h-full" />
      <div className="absolute bottom-2 left-2 text-white text-xs bg-black/30 rounded px-2 py-1 backdrop-blur-sm">
