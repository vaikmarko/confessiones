document.addEventListener('DOMContentLoaded', function () {
  ...
  // SVG ja <g> wrapper
  const svg = d3.select('#inner-space-visualization').append('svg').attr('width', width).attr('height', height);
  ...
  g.selectAll('circle.star').data(stars).enter().append('circle').attr('class', 'star').attr('cx', d => d.x).attr('cy', d => d.y).attr('r', d => d.r).style('fill', 'white').style('opacity', d => d.opacity);
  ...
  function loadStories() {
    fetch('/api/stories').then(response => response.json()).then(data => {
  ...
  function visualizeStories(stories) {
    ...
    // Joonista seosed
    g.selectAll('.connection-line').data(connections).enter().append('line').attr('class', 'connection-line').attr('x1', d => d.source.x).attr('y1', d => d.source.y).attr('x2', d => d.target.x).attr('y2', d => d.target.y).style('stroke', 'rgba(255, 255, 255, 0.2)').style('stroke-width', d => d.strength * 2);

    // Joonista lugude sõlmed
    const storyNodes = g.selectAll('.story-node').data(nodes).enter().append('g').attr('class', 'story-node').attr('transform', d => `translate(${d.x},${d.y})`);

    // Lisa ringid
    storyNodes.append('circle').attr('r', d => d.r).style('fill', d => `hsl(${d.idx * 137.5 % 360}, 70%, 50%)`).style('opacity', 0.8);

    // Lisa tekst
    storyNodes.append('text').text(d => {
      ...
      return 'Untitled';
    }).attr('text-anchor', 'middle').attr('dy', 4).style('fill', 'white').style('font-size', '12px');
    ...
    // Lisa liikumine
    const simulation = d3.forceSimulation(nodes).force('charge', d3.forceManyBody().strength(-100)).force('center', d3.forceCenter(width / 2, height / 2)).force('collision', d3.forceCollide().radius(d => d.r + 10)).on('tick', () => {
      storyNodes.attr('transform', d => `translate(${d.x},${d.y})`);
      g.selectAll('.connection-line').attr('x1', d => d.source.x).attr('y1', d => d.source.y).attr('x2', d => d.target.x).attr('y2', d => d.target.y);
    });
  }

  // Avastuste kuvamine
  function showInsights(storyId) {
    fetch(`/api/insights/${storyId}`).then(response => response.json()).then(data => {
  ...
  function showConnections(storyId) {
    fetch(`/api/connections/${storyId}`).then(response => response.json()).then(data => {
